...
import com.yilin.yixueblog.enums.EPublish;
...
//        }
    ...
    public IPage<Blog> searchBlogByTag(String tagUid, Long currentPage, Long pageSize) {
        ...
        if (tag != null) {
            ...
            String jsonResult = redisUtil.get("TAG_CLICK:" + ip + "#" + tagUid);
            if (StringUtils.isEmpty(jsonResult)) {
                ...
                redisUtil.setEx("TAG_CLICK:" + ip + "#" + tagUid, clickCount + "",
                        24, TimeUnit.HOURS);
            }
        }
        QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
    ...
    public IPage<Blog> searchBlogByBlogSort(String blogSortUid, Long currentPage, Long pageSize) {
        ...
        if (blogSort != null) {
            ...
            String jsonResult = redisUtil.get("SORT_CLICK:" + ip + "#" + blogSortUid);
            if (StringUtils.isEmpty(jsonResult)) {
                //给分类点击数增加
                ...
                blogSort.updateById();
                //将该用户点击记录存储到redis中, 24小时后过期
                redisUtil.setEx("SORT_CLICK:" + ip + "#" + blogSortUid, clickCount + "",
                        24, TimeUnit.HOURS);
            }
        }
